...
namespace AudioMod
{
    class Program
    {
        ...
        {
            bool ExStat = true;
            //So I don't have to see huge Console.WriteLine functions
            Assertions Cnsl = new Assertions();

            //samples, samples per second, audio channels, bits per sample, header length, Console
            AudioManager audio = new AudioManager(226380, 44100, 2, 16, 44, Cnsl);

            //Loads the audio
            AudioManager.AudioSample InputAudio = audio.LoadAudio("Data/AudioParts/SnailsHouse-Morph", true, ref ExStat);
            if (!ExStat) { Cnsl.Main_CoreExecutionHalt(); return; }

            //Audio manager data, InputAudio, steps of resolution from top to bottom, is total steps: false, is memory alloc: true
            audio.GenerateAudioFile("Data/Output/SnailsHouse-Morph_Output", TerraceAudio(audio, InputAudio, 8, false, false), false, ref ExStat);
            if (!ExStat) { Cnsl.Main_CoreExecutionHalt(); return; }
        }
    }
}
